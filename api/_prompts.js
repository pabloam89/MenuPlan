...
- "steps": array de OBJETOS {text, minutes, kind} — no de strings. MUY IMPORTANTE: si el usuario proporciona "preparationNotes" (su propia explicación de cómo lo prepara), tu única labor es reestructurar ESE texto en pasos, en el mismo orden y con la misma técnica que describe — no inventes un método distinto ni lo sustituyas por uno genérico, solo dale formato. Si no hay "preparationNotes", estima los pasos a partir de los ingredientes. Reglas de los pasos:
...
Devuelve el JSON con exactamente estas claves: name, category, mainProtein, mealRole, usageTags, type, time, difficulty, kcal, protein_g, carbs_g, fat_g, baseServings, kidFriendly, tupperFriendly, allergens, season, ingredients (array de {name, amount, unit}), steps (array de {text, minutes, kind} con "during" opcional en los pasos paralelos), description.`,
};
