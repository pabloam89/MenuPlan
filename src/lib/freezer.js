...
/**
 * ¿Aguanta este plato el congelador?
 *
 * `freezable` está curado en 888 de las 1011 recetas del catálogo y hasta el
 * 11 sep 2026 NO LO LEÍA NADIE: el flujo de congelador trabaja sobre raciones
 * ya cocinadas (`cooked_dish` en user_pantry) y nunca preguntó si el plato se
 * podía congelar. Resultado: la app enseñaba `thawSteps` —cómo descongelar—
 * de platos que nunca había comprobado que fueran congelables, y dejaba
 * mandar al congelador un rebozado que vuelve blando.
 *
 * Es estricto a propósito: solo `true` cuenta. Un plato sin el campo (las 123
 * que faltan, casi todas de usuario o de IA) o marcado `false` devuelve false.
 * El riesgo es asimétrico: negar el congelador a un guiso que sí aguanta solo
 * pierde comodidad; ofrecérselo a unas croquetas arruina la cena.
 *
 * Es la puerta que hay que consultar donde se OFRECE congelar (el toggle al
 * marcar cocinado, en Menu.jsx) y donde se elija "congelable" como filtro.
 *
 * @param {object|null|undefined} recipe
 * @returns {boolean}
 */
export function canFreezeRecipe(recipe) {
  return recipe?.freezable === true;
}
