...
import {
  FAMILIAS_COCINADO, FAMILIAS_PLATO, FAMILIAS_SEMI, SIN_FAMILIA,
  familiaDePlato, manosDeTanda,
...
describe("las familias de la tanda", () => {
  ...
  it("reparte los 39 platos a medio hacer entre las doce familias", () => {
    ...
    const repartidos = FAMILIAS_SEMI.reduce((n, f) => n + f.recetas.length, 0);
    expect(repartidos).toBe(marcados);
    expect(FAMILIAS_SEMI).toHaveLength(12);
  });
  ...
  it("las ollas son solo sopas y cremas, y solo las que aguantan", () => {
    ...
    for (const f of FAMILIAS_COCINADO) {
      for (const id of f.recetas) {
  ...
  it("una sopa que se gratina ración a ración no entra", () => {
    const dentro = FAMILIAS_COCINADO.flatMap((f) => f.recetas);
