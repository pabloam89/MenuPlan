...
export const EJES = [
  ...
  },
  {
    ...
    nota: "Un solver lo exige: dos platos al horno a la vez a temperaturas distintas no se pueden. DERIVADO por `recursoDe` de `tecnica` — 634 fuego, 173 horno, 105 ninguno—, y el conflicto se pregunta con `seEstorban(a, b)` porque NO es propiedad de una receta sino de un par. No sale de los marcadores `{{@Aparato}}` aunque parecieran el sitio natural: están en el 42,9 % de las recetas y 386 de 515 son «Sartén», así que un eje derivado de ellos habría contestado «fuego» a casi todo con cara de medido. Tampoco de `requiredAppliance` ni `methods`: el primero dice qué hace falta TENER y el segundo son alternativas que el usuario no ha elegido. — EL MODELO ESTABA MAL HASTA EL 24 SEP, y por eso el eje no lo usaba nadie: `seEstorban` decía que dos platos chocan si reservan el MISMO recurso, y con eso el 49,8 % de los pares del recetario salían en conflicto — el 95 % de ellos por ser «dos de fuego», o sea una olla y una sartén. En una cocina hay cuatro fuegos: eso no es un conflicto, es una comida normal. Faltaba la CAPACIDAD del recurso (horno 1, fuego 4, ninguno ∞), y con ella los pares en conflicto bajan al 2,7 %, que son los que de verdad obligan a cocinar en dos turnos. El test del eje no lo cazó porque probaba horno+horno y horno+crudo —los dos correctos— y nunca olla+sartén. Se añade además `aforoExcedido(recetas)`, porque el PAR no es el caso general: una comida con primero, segundo y guarnición puede llevar tres cosas al horno, y eso son tres turnos aunque ningún par diga nada distinto de lo que dicen dos. — SIGUE SIN LECTOR a propósito: su uso natural era afinar `SOLAPE_COMIDA` en la regla `comida_demasiado_larga`, y medido cambia el veredicto en el 0,29 % de los pares (el horno coincide en el 2,7 %). No compensa una rama nueva en una regla para mover tres casos de cada mil; el eje se deja arreglado para cuando haya un uso que lo pague.",
  },
  {
