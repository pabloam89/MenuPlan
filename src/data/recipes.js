...
// id -> index into RECIPES, kept in sync by registerRecipes. Re-registering
// an already-known recipe (every regeneration re-registers the whole week's
// dishes, plus every single-dish swap) used to do a full RECIPES.findIndex
// scan each time — O(archive size) per recipe, repeated per call, and the
// archive only got bigger once history was raised from 12 to 40 menús.
const RECIPES_INDEX = new Map(RECIPES.map((r, i) => [r.id, i]));
...
export function registerRecipes(extra) {
  ...
  for (const recipe of extra) {
    ...
    if (idx === undefined) {
      RECIPES_INDEX.set(recipe.id, RECIPES.length);
      RECIPES.push(stored);
    } else {
      RECIPES[idx] = stored;
    }
