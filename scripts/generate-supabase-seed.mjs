...
// Valores que el enum meal_role debe tener (migración 0001 solo creó los 5
// originales; 0008 debía añadir desayuno/merienda/postre pero no siempre se
// aplicó; "salsa" no está en ninguna migración). Postgres exige que un valor
// de enum nuevo esté COMMITTED antes de poder usarse — por eso estas van
// sueltas, cada una su propia sentencia autoconfirmada, ANTES del begin/commit
// de las inserciones que ya las necesitan.
const MEAL_ROLE_VALUES = [
  "cena", "guarnicion", "plato_unico", "primero", "segundo",
  "desayuno", "merienda", "postre", "salsa",
];
...
recipeLines.push("");
recipeLines.push("-- Fuera de la transacción a propósito: un valor de enum nuevo debe estar");
recipeLines.push("-- confirmado antes de poder usarse en el INSERT de más abajo.");
for (const v of MEAL_ROLE_VALUES) {
  recipeLines.push(`alter type meal_role add value if not exists '${v}';`);
}
recipeLines.push("");
