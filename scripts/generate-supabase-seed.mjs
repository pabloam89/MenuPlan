...
// aplicó; "salsa" no está en ninguna migración).
...
setupLines.push("");
setupLines.push("-- meal_role: migración 0001 solo creó los 5 originales; 0008 (desayuno/");
setupLines.push("-- merienda/postre) no siempre se aplicó; \"salsa\" no está en ninguna migración.");
for (const v of MEAL_ROLE_VALUES) {
  setupLines.push(`alter type meal_role add value if not exists '${v}';`);
}
setupLines.push("");
